...
contract HostSideValidationStarter {
    ...
    bytes32 internal constant NULLIFIER_DOMAIN_SEPARATOR = keccak256("C05:nullifier:v1");
    ...
    // @audit Why is trusting a host-side policy still unsafe here?
    function submit(bytes calldata proof, bytes calldata publicValues) external {
        ...
        balances[recipient] += uint256(amount);
    }
